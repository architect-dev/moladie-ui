...
import { RetryableError, retry, CancelledError } from '@utils/retry'
import { Call, CallsState } from './callsTypes'
import { useCallsStore } from './callsStore'
import { parseCallKey, chunkArray } from './callsUtils'
 ...
 */
export function activeListeningKeys(allListeners: CallsState['callListeners'], chainId?: number): { [callKey: string]: number } {
...
export function outdatedListeningKeys(
	callResults: CallsState['callResults'],
...
export default function Updater(): null {
	const fetchingMulticallResults = useCallsStore((state) => state.fetchingMulticallResults)
	const updateMulticallResults = useCallsStore((state) => state.updateMulticallResults)
	const errorFetchingMulticallResults = useCallsStore((state) => state.errorFetchingMulticallResults)
	const callListeners = useCallsStore((state) => state.callListeners)
	...
	const chainId = parseFloat(CHAIN_ID) as number
