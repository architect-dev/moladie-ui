import { Column } from '@uikit'
import React from 'react'
import styled, { css, keyframes } from 'styled-components'

const SlideHoriz = keyframes`
  0% { transform: translateX(-5%); }
  50% { transform: translateX(-20%); }
  100% { transform: translateX(-5%); }
`
const SlideVert = keyframes`
  0% { transform: rotate(5deg) translate(-10%, -5%); }
  50% { transform: rotate(-5deg) translate(-10%, -40%); }
  100% { transform: rotate(5deg) translate(-10%, -5%); }
`

const GradientWrapper = styled.div<{ height?: string }>`
	position: relative;
	display: inline-block;
	width: 100%;
	height: ${({ height }) => height ?? '130px'};
	border-radius: 8px;
	/* box-shadow: ${({ theme }) => `0px 5px 10px ${theme.colors.text}40`}; */
	overflow: hidden;
	color: white;
`

const AnimatedGradient = styled.div<{ colors?: [string, string, string]; noAnim: boolean; opacity: number }>`
	position: absolute;
	top: 0;
	left: 0;
	display: inline-block;
	width: 140%;
	height: 200%;
	background-color: #ffecd2;
	overflow: hidden;
	transition: all 0.2s ease-out;
	cursor: pointer;
	isolation: isolate;

	background-color: ${({ colors }) => (colors == null ? '#9dc0ee' : colors[0])};
	transform-origin: center;

	opacity: ${({ opacity }) => opacity};
	transition: opacity 200ms;

	&:before {
		content: '';
		background: repeat-x 100% 0/50% 100% radial-gradient(circle at 55% 0, transparent 0%, ${({ colors }) => (colors == null ? '#2f627d' : colors[1])} 30%);
		position: absolute;
		display: block;
		top: 0;
		left: 0;
		height: 100%;
		width: 400%;
		mask: url(data:image/png;base64,),
			radial-gradient(circle at 55% 0, transparent 0%, #000 80%) 0 0/50% 100%;
	}

	&:after {
		content: '';
		background: repeat-x 100% 0/50% 100% radial-gradient(circle at 55% 0, transparent 25%, ${({ colors }) => (colors == null ? '#1c3053' : colors[2])} 65%);
		position: absolute;
		top: 0;
		display: block;
		top: 0;
		left: 0;
		height: 100%;
		width: 400%;
		mask: url(data:image/png;base64,),
			radial-gradient(circle at 55% 0, transparent 40%, #000 90%) 0 0/50% 100%;
	}

	${({ noAnim }) => {
		if (noAnim) return false
		const maxTime = 120
		const startTime = Math.random() * 120
		return css`
			animation: ${SlideVert} ${maxTime}s ease-in-out infinite;
			animation-delay: -${startTime}s;

			&:before {
				animation: ${SlideHoriz} ${maxTime * 0.63}s ease-in-out infinite;
				animation-delay: -${startTime}s;
			}
			&:after {
				animation: ${SlideHoriz} ${maxTime * 0.63}s ease-in-out infinite;
				animation-delay: -${startTime}s;
			}
		`
	}}
`

export const HighlightedItem: React.FC<{
	children?: React.ReactNode
	colors?: [string, string, string]
	height?: string
	noAnim?: boolean
	opacity?: number
}> = ({ children, colors, height, noAnim = false, opacity = 1 }) => {
	return (
		<GradientWrapper height={height}>
			<AnimatedGradient colors={colors} noAnim={noAnim} opacity={opacity} />
			<Column color='inherit' position='absolute' alignItems='center' justifyContent='center' width='100%' height='100%' zIndex={3}>
				{children}
			</Column>
		</GradientWrapper>
	)
}
